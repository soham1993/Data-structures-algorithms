...
img=cv2.imread('D:/PAP/2d_40X.png')
row,col,n=img.shape
...
plt.subplot(2,1,2), plt.hist(r1.ravel(), 256)
...
cv2.imwrite("D:/PAP/constrast_stretch.jpg", img1)
...
plt.subplot(2,1,2), plt.hist(clahe_output_g.ravel(), 256)
plt.title('Histogram'), plt.xticks([]), plt.yticks([])
...
cv2.imwrite("D:/PAP/sobel.jpg", sobel)
...
cv2.imwrite("D:/PAP/connected componenets.jpg", output)      
#markers=markers.astype('uint8')
#ret4,th4  = cv2.threshold(markers,0,255,cv2.THRESH_BINARY)
#cv2.imwrite("D:/PAP/otsu_CC.jpg", th4)      
min_size =  5755391 
sizes = stats[0:, -1]; 
nb_components = nb_components - 1
#your answer image
img2 = np.zeros((th1.shape))
...
ret3,th3  = cv2.threshold(lab,0,255,cv2.THRESH_BINARY)
...
cv2.imwrite("D:/PAP/final_output_GMM.jpg",Final_output_1 )

"""Creating a mask"""
"""mask = np.zeros_like(Final_output )
mask1=np.zeros_like(Final_output )
mask2=np.zeros_like(Final_output )
mask3=np.zeros_like(Final_output )
rows, cols,_ = mask.shape
# create a white filled ellipse
start=(0,int(col*.75))
end=(int(row*.25),0)     
mask=cv2.rectangle(mask,start,end,(255,255,255),-1)
result = np.bitwise_and(Final_output,mask)

mask=cv2.line(Final_output ,(0,int(col*.40)), (int(row*.75),0), (0, 20, 200), 10)
mask1=cv2.line(Final_output ,(int(row*.75),0),(int(col),int(row*.40)), (0, 20, 200), 10)
mask2=cv2.line(Final_output ,(int(col),int(row*.40)),(int(row*.55),col), (0, 20, 200), 10)
mask3=cv2.line(Final_output ,(0,int(col*.40)),(int(row*.55),int(col*10)), (0, 20, 200), 10)

result = np.bitwise_and(Final_output,mask)
cv2.imwrite("D:/PAP/Masked.jpg",mask )"""
